...
export default function HeaderTools({ language, onLanguage, assisted, onAssisted, onReset, embedded, text }: Props) {
  ...
  return <div className="header-tools" ref={container}>
    <details className="header-menu language-picker" name="header-tools">
      <summary aria-label={text('Change language')}><span aria-hidden="true" className="language-globe">◎</span><span lang={language}>{LANGUAGES.find((item) => item.code === language)?.name}</span><span aria-hidden="true" className="menu-chevron">⌄</span></summary>
    ...
    <details className="header-menu secondary-menu" name="header-tools">
      ...
      <div className="header-menu-panel" role="group" aria-label={text('More options')}>
        <button type="button" onClick={(event) => choose(event, onReset)}>{text('New example')}</button>
        <button type="button" aria-pressed={assisted} onClick={(event) => choose(event, onAssisted)}>{text(assisted ? 'Standard text' : 'Larger text')}{assisted && <span aria-hidden="true">✓</span>}</button>
        {!embedded && <a href="/frame">{text('iPhone frame')} <span aria-hidden="true">↗</span></a>}
      </div>
    </details>
  </div>
}
