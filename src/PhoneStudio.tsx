...
export default function PhoneStudio() {
  ...
  return <div className={`phone-studio${recording ? ' recording-view' : ''}`}>
    <header className="studio-toolbar" hidden={recording}>
      <a href="/demo" className="studio-back">← Back to demo</a>
      <strong className="studio-title">DBT Rescue <span>Mobile frame</span></strong>
      <div className="studio-controls">
        <div className="studio-zoom" role="group" aria-label="Preview zoom">
          <button type="button" aria-label="Zoom out" disabled={zoom <= .5} onClick={() => setZoom((value) => Math.max(.5, value - .1))}>−</button>
          <output aria-label="Preview scale">{Math.round(scale * 100)}%</output>
          <button type="button" aria-label="Zoom in" disabled={zoom >= 1.5} onClick={() => setZoom((value) => Math.min(1.5, value + .1))}>+</button>
          <button type="button" onClick={() => setZoom(1)}>Fit</button>
        </div>
        <button type="button" onClick={() => setReload((value) => value + 1)}>Reload demo</button>
        <button type="button" className="record-view-button" ref={recordButton} onClick={() => setRecording(true)}>Recording view</button>
      </div>
    </header>
    {recording && <button className="exit-recording" onClick={exitRecording}>Exit recording view <kbd>Esc</kbd></button>}
    <main className="studio-stage" ref={stage} aria-label="Interactive mobile preview">
      <div className="phone-space" style={{ width: 414 * scale, height: 868 * scale }}>
        <div className="studio-phone" style={{ '--phone-scale': scale } as CSSProperties}>
